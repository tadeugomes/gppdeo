...
Não é novidade que a pandemia tem gerado uma catástrofe econômica no Brasil e nos estados. O monitoramento do comportamento do mercado de trabalho tornou-se problemático após a pandemia e as [mudanças](https://economia.uol.com.br/noticias/bbc/2021/04/13/pnad-caged-desemprego-brasil-falhas-estatisticas-mercado-trabalho.htm) no Cadastro Geral de Empregos e Desempregos (CAGED) após a entrada em vigor do **Novo Caged** e as alterações na Pesquisa Nacional por Amostra por Domicílio Contínua (Pnad-Contínua) decorrentes da pandemia, que passou a ser realizada por telefone e teve os dados para os estados não disponibilizados por incompatibilidade nas metodologias de coleta dos dados.
...
### **Taxa de Desemprego no Maranhão**
...
taxa<-dados %>%  
  filter(`Variável`=="Taxa de desocupação, na semana de referência, das pessoas de 14 anos ou mais de idade") %>% 
    filter(Trimestres>="201404")


taxa$Trimestre <- factor(taxa$Trimestre, levels = taxa$Trimestre[order(taxa$Trimestres)])

ggplot(data = na.omit(taxa), aes(x=Trimestre, y=Valor, group = 1))+
  geom_line()+
...
### **Condição da Força de Trabalho Maranhense**
...
plot2<- ggplot( data = na.omit(pessoas_sub_ma), aes(x=Trimestre, y=Valor, color = Condicao, group=1))+
  geom_line(size=1)+
  geom_point(size=2)+ 
  xlab('')+ylab('Em Mil')+
  labs(title='',
 ...
 theme_bw()+ theme(axis.text.x=element_text(angle=90, hjust=1))+
  ...
  theme(plot.title = element_text(size = 10, color = "#4169e1"))
ggplotly(plot2)
```
...
### **Ocupação por setor de atividade econômica**
...
po2 = get_sidra(api='/t/5434/n3/21/v/4090/p/all/c693/all')
  ...
  Setor<-Setor %>% 
  filter(Atividade!="Total" & Atividade!="Atividades mal definidas", Atividade!= "Outro serviço") %>% filter(Trimestres>="201404")
  
plot3<- ggplot(data = na.omit(Setor), aes(x=Trimestre, y=Valor, color = Atividade, group=1))+
  geom_line(size=.5)+
  geom_point()+ 
  xlab('')+ylab('Em Mil')+
  labs(title='',
  ...
  theme_bw()+ theme(axis.text.x=element_text(size = 14, angle=90, hjust=1))+
    theme(legend.position="none")+
  theme(legend.title = element_text(color = "#4169e1"),
        plot.title = element_text(size = 14, color = "#4169e1"))
ggplotly(plot3)
```
...
### **Ocupação por posição na ocupação e categoria do emprego**
...
plot4<-ggplot(data = na.omit(po_filtro_2), aes(x=Trimestre, y=Valor, color = `Categoria`, group=1))+
  geom_line(size=.5)+
  geom_point()+ 
  xlab('')+ylab('Em Mil')+
  labs(title='',
  ...
  theme(legend.title = element_text(color = "#4169e1"))+
  theme(legend.position="none")+
  theme(plot.title = element_text(size = 14, color = "#4169e1"))
ggplotly(plot4)
```
...
Uma melhor visualização dos dados pode ser encontrada no dashboard disponível [aqui](https://rpubs.com/tadeu_teixeira/639841).
